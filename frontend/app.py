...
def sistema_erp():
    ...
    if page_id == "dash":
        ...
        with t1: st.dataframe(pd.DataFrame(get_data("relatorios/estoque/")['itens']), use_container_width=True)
        with t2: st.dataframe(pd.DataFrame(get_data("relatorios/lotes_vencimento/")), use_container_width=True)

    # (Mantendo as outras abas essenciais resumidas para caber)
    elif page_id == "pdv":
        ...
        with st.form("nc"): 
            ...
            if st.form_submit_button("Salvar"): requests.post(f"{API_URL}/clientes/", json={"nome":n,"email":e,"telefone":""})
