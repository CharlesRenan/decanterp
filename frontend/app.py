...
def apply_custom_style():
    ...
    <style>
        ...
        .big-font { 
            font-size: 32px !important; font-weight: 800 !important; color: #FFFFFF !important; margin-top: 15px; margin-bottom: 25px;
        }

        /* --- CORREÇÃO NUCLEAR DO MENU (FIM DO VERMELHO) --- */
        /* Força a cor de fundo do item ativo na lista de navegação */
        ...
        /* --- CARDS GLASS (MAIS VISÍVEIS) --- */
        .glass-card {
            /* Aumentei a opacidade do branco para 0.08 para destacar do fundo preto */
        ...
        .card-value { font-size: 38px; font-weight: 800; color: #FFFFFF; margin-top: 0px; text-shadow: 0 0 20px rgba(255,255,255,0.1); }
...
def sistema_erp():
    with st.sidebar:
        ...
        # O SEGREDINHO DO CSS ESTÁ LÁ EM CIMA NO `apply_custom_style`
        sel = option_menu(
            menu_title=None,
        ...
        with c_meta:
            with st.container(border=True):
                ...
                fig_gauge = go.Figure(go.Indicator(
                    ...
                    gauge = {'axis': {'range': [None, 10000], 'tickwidth': 1, 'tickcolor': "#333"}, 'bar': {'color': "#EC4899"}, 'bgcolor': "#262626", 'borderwidth': 0, 'bordercolor': "#333", 'steps': [{'range': [0, 10000], 'color': "#262626"}]}
                ))
    ...
    elif page_id == "rel": header("Relatórios"); st.info("Módulo de PDF Ativo.")
    elif page_id == "cfg": 
