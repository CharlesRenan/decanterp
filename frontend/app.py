...
# Logo Azul Escuro (para fundo claro)
def render_logo_svg(width="50px", color="#4F46E5"):
...
def apply_custom_style():
    ...
    <style>
        ...
        /* Tipografia Escura */
        h1, h2, h3, h4, h5, h6, strong { color: #111827 !important; }
        p, div, label, span, td { color: #6B7280; } /* Cinza médio */
        ...
        /* Item Selecionado (Azul/Roxo vibrante - igual referencia) */
        .nav-link-selected {
        ...
        /* 4. CARDS (SOFT UI - Sombras suaves) */
        ...
        .kpi-value { font-size: 28px; font-weight: 800; color: #111827; margin: 5px 0; }
        .kpi-title { font-size: 12px; font-weight: 700; color: #9CA3AF; text-transform: uppercase; letter-spacing: 0.5px; }
        ...
        .bg-i-indigo { background-color: #EEF2FF; color: #4F46E5; }
        .bg-i-rose { background-color: #FFF1F2; color: #F43F5E; }
        .bg-i-emerald { background-color: #ECFDF5; color: #10B981; }
        .bg-i-amber { background-color: #FFFBEB; color: #F59E0B; }
...
# Card KPI Novo
def card_html(titulo, valor, icon, cor="bg-i-indigo"): 
    st.markdown(f"""
    <div class='kpi-card'>
...
def get_sales_row_html(nome, email, valor):
    ...
    return f"<div class='sales-row'><div style='display:flex;align-items:center;'><div class='avatar-circle'>{iniciais}</div><div><div style='color:#111827; font-weight:700; font-size:14px;'>{nome}</div><div style='color:#9CA3AF; font-size:12px;'>{email}</div></div></div><div style='font-weight:700; color:#4F46E5;'>R$ {valor:,.2f}</div></div>"
...
def tela_login():
    ...
    with c2:
        ...
        # Login em Card Flutuante
        with st.container(border=True):
            st.markdown(f"<div style='display:flex; flex-direction:column; align-items:center; gap: 15px; margin-bottom: 20px;'>{render_logo_svg(width='60px', color='#4F46E5')}<h2 style='margin:0; color:#1F2937 !important;'>Decant ERP</h2></div>", unsafe_allow_html=True)
...
def sistema_erp():
    with st.sidebar:
        st.markdown(f"<div style='display:flex; align-items:center; gap:12px; margin-bottom:30px; padding-left:10px; padding-top:20px;'>{render_logo_svg(width='30px', color='#4F46E5')}<div style='font-family:Manrope; font-weight:800; font-size:22px; color:#1F2937;'>Decant</div></div>", unsafe_allow_html=True)
        ...
        sel = option_menu(None, [x["l"] for x in menu], icons=[x["i"] for x in menu], default_index=0, styles={"container": {"padding": "0!important", "background-color": "transparent"}, "icon": {"font-size": "16px"}})
    ...
    if page_id == "dash":
        ...
        with c1: card_html("Receita", f"R$ {d.get('receita', 0):,.2f}", "💰", "bg-i-indigo")
        with c2: card_html("Despesas", f"R$ {d.get('despesas', 0):,.2f}", "📉", "bg-i-rose")
        with c3: card_html("Lucro", f"R$ {d.get('lucro', 0):,.2f}", "💎", "bg-i-emerald")
        with c4: card_html("Status", "Online", "🟢", "bg-i-amber")
        ...
        with c_graf:
            with st.container(border=True): 
                ...
                if dados:
                    ...
                    # Gráfico Clean com Curvas Suaves
                    fig = px.area(df_g, x="Mês", y="Valor", color="Tipo", line_shape='spline', color_discrete_map={'Entradas': '#4F46E5', 'Saídas': '#F43F5E'})
                else:
                    fig = px.area(pd.DataFrame([{"Mês":"Jan","Valor":0,"Tipo":"Entradas"}]), x="Mês", y="Valor")
                
                # Layout Branco e Limpo
